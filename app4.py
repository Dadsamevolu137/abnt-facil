...
def get_estilos(fonte, fonte_bold):
    TAM        = 12
    LEAD_15    = TAM * 1.5          # espaçamento 1,5 — corpo do texto
    LEAD_SIM   = TAM * 1.0          # espaço simples — citações longas, refs, notas

    # Corpo do texto: Arial 12, espaço 1,5, recuo 1,25 cm na 1ª linha, justificado
    ...
    # Seção primária: negrito, alinhado à esquerda, separado do texto por 1,5 linha
    # NBR 14724:2024 §5.2.2: títulos das seções primárias começam em nova página
    ...
    # Subseção: mesmo espaçamento antes e depois
    subsecao = ParagraphStyle("subsecao", parent=secao,
        fontName=fonte_bold, spaceBefore=LEAD_15, spaceAfter=LEAD_15)

    # Resumo: título centralizado sem indicativo numérico (NBR 14724:2024 §5.2.3)
    resumo_titulo = ParagraphStyle("resumo_titulo", parent=centralizado_bold,
        spaceBefore=0, spaceAfter=LEAD_15)

    # Texto do resumo: espaço 1,5, sem recuo, justificado (NBR 6028:2021)
    resumo_texto = ParagraphStyle("resumo_texto", parent=base,
        firstLineIndent=0, alignment=TA_JUSTIFY)

    # Referências: espaço simples, sem recuo; separadas entre si por linha em branco
    # NBR 14724:2024 §5.2: referências em espaço simples
    referencia = ParagraphStyle("referencia", parent=base,
        fontName=fonte, fontSize=TAM, leading=LEAD_SIM,
        firstLineIndent=0, alignment=TA_JUSTIFY,
        spaceBefore=0, spaceAfter=LEAD_SIM)   # spaceAfter = 1 linha em branco entre refs

    italico = ParagraphStyle("italico", parent=centralizado,
        fontName=fonte.replace("Helvetica", "Helvetica-Oblique").replace("Arial", "Arial"))

    # Citação longa: fonte menor (10pt), espaço simples, recuo 4 cm esquerda, sem aspas
    # NBR 14724:2024 §5.1 + NBR 10520:2023
    citacao_longa = ParagraphStyle("citacao_longa", fontName=fonte, fontSize=10,
        leading=10 * 1.0,                    # espaço simples
        textColor=colors.black, alignment=TA_JUSTIFY,
...
def gerar_pdf_abnt(data, filepath, logo_path=None):  # noqa: C901
    ...
    # NBR 14724:2024 §5.2: "natureza do trabalho deve ser alinhada do meio da mancha
    # gráfica até a margem direita"
    nota_estilo = ParagraphStyle("nota_direita", parent=estilos["base"],
        fontSize=12, leading=12,              # espaço simples
        firstLineIndent=0,
        alignment=TA_JUSTIFY,
        leftIndent=largura_util * 0.5)        # metade da mancha = do centro até a direita
    ...
    def _on_conteudo(c, doc):
        """Páginas de conteúdo: numeração no canto superior direito a partir da 1ª seção.
        NBR 14724:2024 §5.3: algarismos arábicos, canto superior direito,
        a 2 cm da borda superior, último algarismo a 2 cm da borda direita."""
        inicio = pagina_inicio[0]
        if inicio and doc.page >= inicio:
            numero = (doc.page - inicio) + 2
            c.saveState()
            c.setFont(fonte, 10)
            # 2 cm da borda superior = PG_H - 2cm; 2 cm da borda direita = PG_W - 2cm
            c.drawRightString(PG_W - 2*cm, PG_H - 2*cm, str(numero))
            c.restoreState()

    # ------------------------------------------------------------------ #
    #  Monta o documento                                                  #
    # ------------------------------------------------------------------ #
    # Frame de conteúdo com topPadding extra para não colidir com o nº de página
    # NBR 14724:2024 §5.3: número a 2 cm da borda → deixamos 2.5 cm de topo
    frame_pg = Frame(MARGEM_ESQ, MARGEM_BOT, largura_util, altura_util,
                     leftPadding=0, rightPadding=0,
                     topPadding=0.5*cm, bottomPadding=0)
    ...
    # Seções — NBR 14724:2024 §5.2.2: títulos primários começam em nova página
    from reportlab.platypus import KeepTogether
    for i, secao in enumerate(secoes_validas):
        tit = secao.get("titulo", "").strip()
        txt = secao.get("conteudo", "").strip()
        # Cada seção primária inicia em nova página
        bloco = [PageBreak(), Paragraph(f"{i+1}. {tit.upper()}", estilos["secao"])]
        for paragrafo in txt.split("\n\n"):
            paragrafo = paragrafo.strip()
            if not paragrafo:
                continue
            bloco.extend(processar_paragrafo(paragrafo, estilos, largura_util))
        # Garante título + primeiro elemento juntos
        story.append(KeepTogether(bloco[:3] if len(bloco) > 3 else bloco))
        for item in bloco[3:]:
            story.append(item)

    # Referências — NBR 14724:2024 §5.2.3: título sem indicativo, centralizado
    # §5.2: referências em espaço simples, separadas entre si por linha em branco
    if referencias:
        titulo_refs = Paragraph("REFERÊNCIAS", estilos["resumo_titulo"])
        refs_bloco = [PageBreak(), titulo_refs, Spacer(1, 0.3 * cm)]
...
def index():
    ...
    pdfs_gerados = usuario.get("pdfs_gerados", 0)
    limite       = usuario.get("limite_pdfs", 99)
    return render_template("index.html",
        usuario=usuario,
        pode_gerar=pode_gerar_pdf(usuario),
        ano=datetime.now().year,
        free_limit=limite,
        guest_count=pdfs_gerados)
