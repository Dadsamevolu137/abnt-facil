...
def get_estilos(fonte, fonte_bold):
    ...
    # Citação longa ABNT: recuo 4cm esquerda, fonte 10pt, sem indent, justificado
    citacao_longa = ParagraphStyle("citacao_longa", fontName=fonte, fontSize=10,
        leading=10 * 1.5, textColor=colors.black, alignment=TA_JUSTIFY,
...
def gerar_pdf_abnt(data, filepath, logo_path=None):  # noqa: C901
    ...
    def _on_conteudo(c, doc):
        """Páginas de conteúdo: numeração no rodapé direito a partir da 1ª seção."""
        inicio = pagina_inicio[0]
        if inicio and doc.page >= inicio:
            numero = (doc.page - inicio) + 2
            c.saveState()
            c.setFont(fonte, 10)
            c.drawRightString(PG_W - MARGEM_DIR, 1.0 * cm, str(numero))
            c.restoreState()
    ...
    for i, secao in enumerate(secoes_validas):
        ...
        # Garante que pelo menos título + primeiro elemento ficam juntos
        story.append(KeepTogether(bloco[:3] if len(bloco) > 3 else bloco))
        for item in bloco[3:]:
            story.append(item)

    # Referências — sem PageBreak, flui junto com as seções
    if referencias:
        refs_bloco = [
            Paragraph(f"{len(secoes_validas)+1}. REFERÊNCIAS BIBLIOGRÁFICAS", estilos["secao"]),
            Spacer(1, 0.3 * cm),
        ]
