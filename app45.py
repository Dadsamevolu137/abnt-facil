...
def gerar_pdf_abnt(data, filepath, logo_path=None):  # noqa: C901
    ...
    for i, secao in enumerate(secoes_validas):
        ...
        bloco = [Paragraph(f"{i+1}. {tit.upper()}", estilos["secao"])]
    ...
    # Referências — título centralizado sem indicativo numérico (NBR 14724:2024 §5.2.3)
    if referencias:
        refs_bloco = [Paragraph("REFERÊNCIAS", estilos["resumo_titulo"]),
                      Spacer(1, 0.3 * cm)]
    ...
    # Anexos — cada um em nova página, título centralizado (NBR 14724:2024 §5.8)
    ...
    for idx, anexo in enumerate(anexos_validos):
        ...
        story.append(Paragraph(rotulo, estilos["resumo_titulo"]))

        imagem_b64 = anexo.get("imagem", "").strip()
        if imagem_b64 and imagem_b64.startswith("data:image"):
            try:
                # Extrai os bytes da imagem base64
                header, data_b64 = imagem_b64.split(",", 1)
                img_bytes = base64.b64decode(data_b64)
                img_buffer = _io.BytesIO(img_bytes)
                img_flowable = Image(img_buffer, width=largura_util * 0.85, height=largura_util * 0.55, kind="proportional")
        ...
        legenda_estilo = ParagraphStyle("legenda_anexo", parent=estilos["base"],
            fontSize=10, leading=14, firstLineIndent=0, alignment=TA_CENTER,
            spaceBefore=0.3 * cm, spaceAfter=0.1 * cm)
        fonte_estilo = ParagraphStyle("fonte_anexo", parent=legenda_estilo,
            fontSize=9, leading=12, textColor=colors.HexColor("#555555"))

        if legenda:
            story.append(Paragraph(legenda, legenda_estilo))
        if fonte_anexo:
            story.append(Paragraph(f"Fonte: {fonte_anexo}", fonte_estilo))
