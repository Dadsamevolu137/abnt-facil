...
def converter_para_abnt():
    ...
    if not (nome.endswith(".docx") or nome.endswith(".pdf")):
        return jsonify({"erro": "Apenas arquivos .docx ou .pdf são aceitos"}), 400
    ...
    try:
        # Extrai texto e seções do arquivo
        secoes = []
        texto_completo = ""
        ...
        if nome.endswith(".docx"):
            ...
            for para in doc.paragraphs:
                ...
                # Detecta título de seção: parágrafo em negrito ou estilo Heading
                eh_titulo = False
            ...
            with _pdfplumber.open(arquivo) as pdf:
                for page in pdf.pages:
                    t = page.extract_text()
                    if t:
                        texto_linhas.append(t)
            texto_completo = "\n".join(texto_linhas)

            # Para PDF sem detecção de negrito, trata como seção única
            secoes = [{"titulo": "Conteúdo", "conteudo": texto_completo.strip()}]

        if not secoes:
            return jsonify({"erro": "Não foi possível extrair conteúdo do arquivo. Verifique se o arquivo não está vazio."}), 400

        # Gera o PDF em ABNT
        dados_capa["secoes"]   = secoes
        dados_capa["resumo"]   = ""
        dados_capa["palavras_chave"] = ""
        dados_capa["referencias"]    = ""
        dados_capa["campus"]   = ""
        dados_capa["disciplina"] = ""
        dados_capa["turma"]    = ""
        dados_capa["mes"]      = ""
        dados_capa["anexos"]   = []
